...
todos:
  ...
  - id: wi-3-editor
    ...
    status: pending
  - id: wi-4-activation
    content: "WI-4: Activation event -- add workspaceContains:**/.bob/custom_modes.yaml to package.json"
    status: pending
  - id: wi-5-channel-api
...
- `BobModeCodeLensProvider` registered for `**/.bob/custom_mode{s,}.{yaml,yml}`
...
- Add `when` clause: `kaoto.bobModesFileExists` so the view only appears when a workspace has `.bob/custom_modes.yaml`
...
- Each `BobModeItem` displays the mode `name` as label and `slug` as description
...
**Welcome content (empty state -- no `.bob/custom_modes.yaml` in workspace):**
...
- `when: "workspaceFolderCount > 0 && !kaoto.bobModesFileExists"` -- workspace open but no modes file
...
- Sets context key `kaoto.bobModesFileExists` based on `findFiles('**/.bob/custom_modes.yaml')` + `FileSystemWatcher`
...
2. Extension shows `showInputBox` with default value: `Switch to <mode-name> mode. `
3. On confirmation, the prompt is sent to Bob via `sendMessageWithHiddenPrompt(undefined, prompt)`
4. The mode-switch prefix is added automatically in the background
...
### WI-2: CodeLens for `custom_modes.yaml` Source Editor (Dominik)
...
- Registered for `{ language: 'yaml', pattern: '**/.bob/custom_mode{s,}.{yaml,yml}' }`
- Two lenses per slug line: `Open in Kaoto` and `Try with Kaoto`
- "Try with Kaoto" follows the same prompt flow as the tree view: shows input box with `Switch to <mode-name> mode` prefix, sends to Bob
...
- Add `{ "filenamePattern": "custom_modes.yaml" }` to `customEditors[0].selector` in `package.json`
...
### WI-4: Activation Event (Lars)

**package.json change:**

- Add `"workspaceContains:**/.bob/custom_modes.yaml"` to `activationEvents`
...
- Extension shows `vscode.window.showInputBox` with `Switch to <modeName> mode. ` prefilled
...
- UI team calls `channelApi.tryBobMode(slug, name, userPrompt)` with the prompt already provided
- Extension just sends it to Bob, no input box needed
...
| File                                         | Action                                                    | Owner                             |
| -------------------------------------------- | --------------------------------------------------------- | --------------------------------- |
| `package.json`                               | Add view, commands, menus, activation event               | Lars (WI-1, WI-4), Dominik (WI-2) |
...
Extension team owns: WI-1 (tree view), WI-2 (CodeLens), WI-4 (activation event), WI-3 package.json selector, WI-5 extension-side method prototype, and the Bob chat integration wiring. The Kaoto UI team owns WI-3 canvas rendering and WI-5 UI-side button.
...
- **Lars** -- starts implementation while Dominik is out. Owns the tree view (WI-1) and activation event (WI-4).
...
1. **WI-4: Activation event** -- add `"workspaceContains:**/.bob/custom_modes.yaml"` to `package.json` activationEvents. Smallest change, good starting point to verify the build pipeline.

2. **WI-1: Bob IDE Modes tree view** -- the main work item:
...
3. **Bob chat integration** -- contribute `SendPromptToChatCommand.ts`, `TestBobIntegrationCommand.ts`, and their constants/registration. Wire Lars's "Try It..." stub to the real `sendMessageWithHiddenPrompt` call. The key call: `vscode.commands.executeCommand('bob-code.sendMessageWithHiddenPrompt', undefined, promptText)`.

4. **WI-2: CodeLens** -- contribute `BobModeCodeLensProvider.ts` with "Open in Kaoto" and "Try with Kaoto" lenses above `- slug:` lines, ensure shared prompt logic between tree view and CodeLens.

5. **WI-5: Channel API method prototype** -- add `tryBobMode(slug, modeName, prompt?)` to `VSCodeKaotoEditorChannelApi.ts` on the extension side. This is the bridge the Kaoto UI team will call from their canvas. The extension-side implementation calls the same `sendMessageWithHiddenPrompt` path. The Kaoto UI team only needs to invoke this method from their button -- they don't touch Bob commands. See WI-5 section for Option A vs Option B UX decision.

6. **Integration polish + cleanup** -- review Lars's tree view, add `package.json` command declarations for all Bob commands, remove any exploratory code.

### Integration contract between Lars and Dominik

Lars creates the "Try It..." command handler as a simple function that receives `{ slug, name, prompt }` and does a placeholder action. Dominik later replaces the body with the Bob chat call. Agreed interface:
...
export async function tryBobMode(slug: string, modeName: string): Promise<void> {
  const promptText = await vscode.window.showInputBox({
    ...
    prompt: "Edit the prompt, then press Enter to send to Bob. Press Escape to cancel.",
    value: `Switch to ${modeName} mode. `,
    ignoreFocusOut: true,
  });
  if (!promptText) return;

  // STUB -- Dominik replaces this with bob-code.sendMessageWithHiddenPrompt
  KaotoOutputChannel.logInfo(`[stub] Would send to Bob: ${promptText}`);
  vscode.window.showInformationMessage(`Prompt ready: ${promptText}`);
}
```
...
- For the prompt, auto-prefix with `Switch to <mode-name> mode. ` so the user sees it pre-filled and can append their own text.
...
- If `prompt` is omitted: extension shows a VS Code input box for the user to type a prompt (Option A)
- If `prompt` is provided: extension sends it directly to Bob, no input box (Option B -- use this if the UI team wants to render a styled input in the canvas)
- Auto-prefixes with `Switch to <modeName> mode. ` in both cases
