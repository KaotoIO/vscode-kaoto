...
export class CamelTestRunTask extends CamelTask {
	...
	static async create(filePath: string): Promise<CamelTestRunTask> {
		const result = await CamelCommandAPI.testRun(basename(filePath), dirname(filePath));
		return new CamelTestRunTask(result.execution, filePath);
	}
}
